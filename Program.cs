...
builder.Services.AddScoped<VendaService>(); // Adicionando VendaService
builder.Services.AddScoped<DepositoService>(); // Adicionando DepositoService
...
builder.Services.AddAuthentication(JwtBearerDefaults.AuthenticationScheme)
    .AddJwtBearer(options =>
    {
        options.TokenValidationParameters = new TokenValidationParameters
        {
            ...
            IssuerSigningKey = new SymmetricSecurityKey(Encoding.ASCII.GetBytes("chave-super-secreta-12345"))
        };
    });
...
app.MapGet("/rotaSegura", async (HttpContext context) =>
{
    ...
    var key = Encoding.ASCII.GetBytes("chave-super-secreta-12345abcdefghijklmnop");
...
app.MapPost("/depositos/{depositoId}/produtos/{produtoId}", async (int depositoId, int produtoId, DepositoService depositoService, ProductService productService) =>
{
    try
    {
        ...
        // Cria um objeto ProdutoDeposito para associar ao depósito
        ...
        // Chama o método no serviço de depósito para associar o produto ao depósito
        await depositoService.DepositProdutoAsync(depositoId, produtoDeposito);

        // Retorna 201 Created com o produto associado ao depósito
        return Results.Created($"/depositos/{depositoId}/produtos/{produtoId}", produto);
    }
    catch (KeyNotFoundException)
    {
        // Retorna 404 Not Found se o depósito ou o produto não forem encontrados
        return Results.NotFound($"Depósito or Product not found.");
    }
    catch (Exception ex)
    {
        // Loga a exceção para diagnóstico
        Console.WriteLine($"An error occurred: {ex.Message}");

        // Retorna um código de status 500 com uma mensagem genérica de erro interno
